                ...
                if distance is not None and distance < 200:   # Check if distance is valid (not None) and less than 40
                    if not speech_played:                    # Check if speech has already been played for this detection
                        try: 
                            ...
                            data = f"Welcome to Smart City Living Lab. We have deployed over three hundred sensor nodes all over the campus and we also have wi sun backbone network in place. The current value of CO2 is {con_value[1]}, temperature is {con_value[2]}, and humidity is {con_value[3]}."
                            text_to_speech(data)
